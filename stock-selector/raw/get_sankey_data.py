...
def get_sankey_data(df, currency):
    ...
    for year in df["calendarYear"]:
        ...
        node_data["x"] = [0.1, 0.3, 0.3, 0.5, 0.5, 0.7, 0.9, 0.9]
        node_data["y"] = [0.6, 0.4, 0.9, 0.3, 0.6, 0.2, 0.1, 0.35]
        node_data["label"] = [
            f"Total<br>Revenue<br>{currency}{format_number(mask.revenue)}",
            f"Gross Profit<br>{currency}{format_number(mask.grossProfit)}",
            f"Cost of<br>Sales<br>{currency}{format_number(mask.costOfRevenue)}",
            f"Operating<br>Income<br>{currency}{format_number(mask.operatingIncome)}",
            f"Operating<br>Expenses<br>{currency}{format_number(mask.operatingExpenses)}",
            f"Pre-tax<br>Income<br>{currency}{format_number(mask.incomeBeforeTax)}",
            f"Net<br>Income<br>{currency}{format_number(mask.netIncome)}",
            f"Tax<br>{currency}{format_number(mask.incomeTaxExpense)}"
        ]
        node_data["color"] = [
            "#FF72D9",
            "#4BDB95",
